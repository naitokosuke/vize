        ...
        TemplateChildNode::TextCall(text_call) => {
            if collect_used_ids {
                match &text_call.content {
                    vize_compiler_core::ast::TextCallContent::Interpolation(interp) => {
                        extract_identifiers_from_expression(&interp.content, &mut result.used_ids);
                    }
                    vize_compiler_core::ast::TextCallContent::Compound(compound) => {
...
fn extract_identifiers_from_compound(
    node: &vize_compiler_core::ast::CompoundExpressionNode,
    ids: &mut FxHashSet<String>,
) {
    ...
    for child in node.children.iter() {
        match child {
            vize_compiler_core::ast::CompoundExpressionChild::Simple(simple) => {
                extract_identifiers_from_simple_expression(simple, ids);
            }
            vize_compiler_core::ast::CompoundExpressionChild::Compound(compound) => {
                extract_identifiers_from_compound(compound, ids);
            }
            vize_compiler_core::ast::CompoundExpressionChild::Interpolation(interp) => {
