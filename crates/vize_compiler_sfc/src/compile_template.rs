...
pub(crate) fn compile_template_block(
    ...
    dom_opts.mode = vize_compiler_core::options::CodegenMode::Module;
    ...
    if let Some(script_bindings) = bindings {
        ...
        for (name, binding_type) in &script_bindings.bindings {
            ...
            binding_map.insert(
                vize_allocator::String::from(name.as_str()),
                vize_allocator::String::from(type_str),
            );
        }
        dom_opts.binding_metadata = Some(vize_compiler_dom::BindingMetadataMap {
    ...
    // Compile template
    let (_, errors, result) =
        vize_compiler_dom::compile_template_with_options(&allocator, &template.content, dom_opts);
