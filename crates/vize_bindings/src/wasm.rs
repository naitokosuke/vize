...
use vize_compiler_dom::{compile_template_with_options, DomCompilerOptions};
...
fn compile_internal(
    ...
    // VDOM mode - use vize_compiler_dom which includes proper v-model transform
        ...
        match child {
            TemplateChildNode::Element(el) => {
                let props: Vec<serde_json::Value> = el
                    ...
                    .iter()
                    .map(|prop| match prop {
                        vize_compiler_core::PropNode::Attribute(attr) => serde_json::json!({
                            "type": "ATTRIBUTE",
                            "name": attr.name.as_str(),
                            "value": attr.value.as_ref().map(|v| v.content.as_str()),
                        }),
                        vize_compiler_core::PropNode::Directive(dir) => serde_json::json!({
                            "type": "DIRECTIVE",
                            "name": dir.name.as_str(),
                            "arg": dir.arg.as_ref().map(|a| match a {
                                vize_compiler_core::ExpressionNode::Simple(exp) => exp.content.as_str().to_string(),
                            ...
                            "modifiers": dir.modifiers.iter().map(|m: &vize_compiler_core::SimpleExpressionNode| m.content.as_str()).collect::<Vec<_>>(),
                        }),
                    })
                    .collect();
